function requireFunction(fn, name) {
    if (typeof fn !== 'function') {
        throw new Error(`Search use cases require a ${name} function`);
    }
    return fn;
}
...
/**
 * Use cases Search : recherche directe par mot-clé et recherche hybride
 * (directe + expansion sémantique via IA) dans l'historique de messages
 * archivés.
 *
 * Dépendances injectées pour ne pas dépendre directement d'une
 * implémentation concrète de la persistance ou du fournisseur IA.
 */
export function createSearchUseCases({ searchArchiveByKeyword, callAI, logSafeError }) {
    requireFunction(searchArchiveByKeyword, 'searchArchiveByKeyword');
    requireFunction(callAI, 'callAI');
    requireFunction(logSafeError, 'logSafeError');

    async function semanticSearch(query, userId = 'legacy') {
        try {
...
{"keywords":["..."]}`,
    ...
    async function hybridSearch(query, userId = 'legacy') {
    ...
    return { semanticSearch, hybridSearch };
}
