import { createAIProviderService } from './ai-provider-service.js';
...
/**
 * Service applicatif Hakili : contient les prompts et toute la logique
 * métier IA (résumé, urgence, brouillons, chat, réglages).
 * Ne connaît ni Groq ni Gemini directement — il délègue l'appel réel au
 * aiProviderService construit ici à partir de getSetting (voir
 ...
 */
export function createHakiliAIService({ getSetting }) {
  if (typeof getSetting !== 'function') {
    throw new Error('createHakiliAIService requires a getSetting function');
  }

  const { callAI } = createAIProviderService({ getSetting });


  async function summarizeMessages(messages, userId = 'legacy') {
  ...
  async function answerSearchQuery(question, messages, userId = 'legacy') {
  if (messages.length === 0) {
  ...
  async function confirmUrgency(message, userId = 'legacy') {
  const raw = await callAI(
  ...
  async function generateDraftReply({ sender, recentHistory, incomingContent, userId = 'legacy' }) {
  ...
  async function chatReply({ conversationHistory, userMessage, archiveContext, userId = 'legacy' }) {
...
- ai_provider_priority: "groq" ou "gemini" — quel fournisseur IA est essayé en premier`;
  ...
  async function parseSettingsIntent(text, currentSettings) {
  const raw = await callAI(
